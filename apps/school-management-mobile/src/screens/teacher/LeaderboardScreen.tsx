...
import {
    ...
    Image, Dimensions, Animated, Pressable, RefreshControl, Modal, Platform
} from 'react-native';
...
export default function TeacherLeaderboardScreen({ navigation }: any) {
    ...
    const fetchClasses = async () => {
        ...
        try {
            ...
            const { data } = await query.order('class_name', { ascending: true });
            setClasses(data ?? []);
        } catch (e) {
    ...
    const handleClassSelect = async (cls: SchoolClass) => {
        ...
        try {
            ...
            if (activeYearId) {
                studentsQuery = studentsQuery.eq('academic_year_id', activeYearId);
            }
            ...
            if (activeYearId) {
                examsQuery = examsQuery.eq('academic_year_id', activeYearId);
            }
            ...
            if (filteredExams.length > 0) {
                ...
                fetchLeaderboard(latestExam.id, cls.id);
            } else {
                setLoading(false);
            }
        } catch (e) {
    ...
    return (
        <LinearGradient colors={['#0F0B20', '#05030A']} style={styles.container}>
            ...
            <View style={styles.header}>
                ...
                <View style={{ flex: 1, alignItems: 'center' }}>
                    ...
                    <Text style={styles.headerTitle} numberOfLines={1}>
                        {selectedClass ? `${selectedClass.class_name} ${selectedClass.section || ''}` : 'Class Zones'}
                    </Text>
                </View>
                <View style={{ width: 40 }} />
            </View>
            ...
            {selectedClass && activeExam && !loading && (
            ...
            {loading && !refreshing ? (
                <View style={styles.centerContainer}>
                    ...
                    <Text style={styles.loadingText}>SYNCHRONIZING ARENA...</Text>
                </View>
            ) : !selectedClass ? (
                <ScrollView
                ...
                >
                    <Text style={styles.classesTitle}>SELECT CLASS ZONE</Text>
                    <View style={styles.classGrid}>
                        {classes.map((cls) => (
                            ...
                            >
                                <LinearGradient
                                    colors={['rgba(35, 30, 70, 0.8)', 'rgba(20, 15, 45, 0.8)']}
                                    style={styles.classGradient}
                                >
                                    <Ionicons name="ribbon-outline" size={32} color="#00F2FE" style={styles.classIcon} />
                                    <Text style={styles.classCardTitle}>{cls.class_name}</Text>
                                    <Text style={styles.classCardSec}>Section {cls.section || 'A'}</Text>
                                </LinearGradient>
                            </TouchableOpacity>
                        ))}
                    </View>
                                        ...
                                        <Animated.View style={[
                                            ...
                                            <View style={[styles.cardExoskeleton, { borderColor: rankStyles.borderColor, shadowColor: rankStyles.borderColor, backgroundColor: rankStyles.panelBg }]}>
                                                ...
                                                <View style={[styles.hackerNameBanner, { backgroundColor: rankStyles.panelBg, borderColor: rankStyles.borderColor }]}>
                                                    ...
                                                    <Text style={[styles.hackerSubText, { color: rankStyles.subTextColor }]}>{(selectedClass?.class_name || 'CLASS').toUpperCase()}</Text>
                                                </View>
...
const styles = StyleSheet.create({
    ...
    classesScrollContent: { padding: 20, paddingBottom: 100 },
    classesTitle: { color: '#FF6A00', fontSize: 12, fontWeight: '800', letterSpacing: 2, marginBottom: 20, textAlign: 'center' },
    classGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' },
    classCard: { width: '48%', height: 140, marginBottom: 15, borderRadius: 16, overflow: 'hidden' },
    classGradient: { flex: 1, padding: 15, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: 'rgba(255, 106, 0, 0.15)', borderRadius: 16 },
    classIcon: { marginBottom: 8 },
    classCardTitle: { color: '#FFFFFF', fontSize: 18, fontWeight: '800' },
    classCardSec: { color: 'rgba(255,255,255,0.4)', fontSize: 12, marginTop: 2 },
    ...
    hackerHeaderTextLeft: {
        fontSize: 10,
        fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace',
        fontWeight: 'bold',
        color: '#00ff66'
    },
    hackerHeaderTextRight: {
        fontSize: 10,
        fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace',
        fontWeight: 'bold',
        color: '#00ff66'
    ...
    hackerFooterTextLeft: {
        fontSize: 8,
        color: 'rgba(0, 255, 102, 0.65)',
        fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace'
    },
    hackerFooterTextRight: {
        fontSize: 8,
        color: 'rgba(0, 255, 102, 0.65)',
        fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace'
    },
